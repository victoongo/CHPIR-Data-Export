...
yaml_files.each do |yaml_file|
  ...
  yaml_name = current_file.split('.').first
	...
	if File.file?("#{yaml_path}/#{yaml_name}.yml")
	survey = hashes2ostruct(YAML.load_file("#{yaml_path}/#{yaml_name}.yml"))
	else
	survey = hashes2ostruct(YAML.load_file("#{yaml_path}/#{yaml_name}.yaml"))
	end
